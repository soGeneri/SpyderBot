...
:: Enable virtual terminal processing so ANSI codes render
...
:: Try 'python' first (standard Windows install from python.org)
python --version >nul 2>&1
if %errorlevel% equ 0 (
    for /f "tokens=2" %%V in ('python --version 2^>^&1') do set PYVER=%%V
    :: Confirm it's Python 3, not Python 2
...
if %errorlevel% neq 0 (
    echo %YELLOW%[!!]%NC%  opencv-contrib-python install failed ^(face recognition may not work^)
) else (
...
:: Check if file is missing or empty (for /f won't set var if file is empty)
set CURRENT_IP=
if exist "%IP_FILE%" (
